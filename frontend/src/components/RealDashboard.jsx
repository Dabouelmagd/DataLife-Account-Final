...
const RealDashboard = () => {
  ...
  return (
    ...
    <div className="flex h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {/* Main Content */}
      <div className={`flex-1 min-h-0 flex flex-col overflow-y-auto transition-all duration-300
        ...
        <div className="flex items-center gap-3 px-4 py-2 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 sticky top-0 z-30">
          ...
          {(() => {
            const role = user?.role || (() => { try { return JSON.parse(localStorage.getItem('user') || '{}').role; } catch { return ''; } })();
            return (role === 'Super Admin' || user?.is_platform_admin) ? (
              <button
