            ...
            <div className="w-10 h-10 rounded-lg overflow-hidden border-2 border-cyan-500 dark:border-cyan-400">
              {(user?.profile_photo_url || user?.profile_photo) ? (
                <img src={(() => {
                  ...
                  onError={e => { e.target.onerror = null; e.target.style.display = 'none'; if (e.target.parentElement) { e.target.parentElement.innerHTML = '<div class=\'w-full h-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-white font-bold text-sm\'>' + ((user?.full_name || 'U').split(' ').map(n => n[0]).join('').toUpperCase().slice(0,2)) + '</div>'; } }} />
              ) : (
        ...
        <nav className="flex-1 overflow-y-auto min-h-0 px-2 py-2">
          ...
          {otherModules.filter(m => m.id !== 'dashboard' && m.id !== 'settings').length > 0 && (
            <div className="mt-2 pt-2 border-t border-gray-200 dark:border-gray-800">
              ...
              {otherModules.filter(m => ['import','user-guide','super-admin','referral','taxes'].includes(m.id)).map((module) => {
                const isActive = activeModule === module.id;
                const colors = getModuleColor(module.id);
                return (
                  <button
